...
def main(training_file, testing_file, generate):
	...
	if(generate):
		...
		for sentence in sentence_list:
			for previous, current in zip(sentence[:-1], sentence[1:]):
				if previous.word not in transitions:
					transitions[previous.word] = {}
				if current.word not in transitions[previous.word]:
					transitions[previous.word][current.word] = 0
				transitions[previous.word][current.word] += 1

		# normalize the counts
		for structure in [transitions]:
		...
		while(repeat):
			if(first):
				testVar = input("Please enter a seed word: ")
				sentenceBuilt += testVar
				previousWord = testVar
				first = False
			else:
				testVar = input("Type 'q' or 'quit' to quit, otherwise the generation will continue: ")

			if(testVar == "Quit" or testVar == "quit" or testVar == "q"):
				repeat = False
			else:
				if (previousWord not in transitions):
					print("Oh no!")
					return
				my_list = []
				for key2 in transitions[previousWord]:
					my_list += [key2] * int(100 * transitions[previousWord][key2])
				chosenWord = random.choice(my_list)
				sentenceBuilt += " " + chosenWord
...
if __name__ == "__main__":
	...
	main(args.training, args.testing, args.generate)
